...
export function Window({
  ...
  const defaults = isBrowserType
    ? DEFAULT_BROWSER_DIMENSIONS
    : DEFAULT_APP_DIMENSIONS;
  ...
  const getCenteredX = () => {
    ...
    return Math.max(
      50,
      (window.innerWidth - windowWidth) / 2 + cascadeOffset
    );
  };
  ...
  const getCenteredY = () => {
    ...
    return Math.max(
      30,
      (window.innerHeight - windowHeight) / 2 - 50 + cascadeOffset
    );
  };
  ...
  const [snapZone, setSnapZone] = useState<"top" | "left" | "right" | null>(
    null
  );
    ...
    const handleMouseMove = (e: MouseEvent) => {
      if (dragRef.current.needsRestore) {
        const deltaX = Math.abs(
          e.clientX - dragRef.current.initialClientX
        );
        const deltaY = Math.abs(
          e.clientY - dragRef.current.initialClientY
        );
            ...
            >
              ...
              {isBrowser ? (
                <div className="flex-1 flex items-center gap-2 ml-3">
                  {(config as BrowserWindowConfig).showReloadButton !==
                    false && (
                    <button
...
function AppContent({ config }: { config: AppWindowConfig }) {
  const Component = config.component as ComponentType<
